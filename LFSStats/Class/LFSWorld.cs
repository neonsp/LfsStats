        ...
        internal static bool Initialize(string pubStatIDkey)
        {
            ...
            while (json == null && requestNumber < maxRequests)
            {
                try
                {
                    ...
                    request.ReadWriteTimeout = 10000; // 10 seconds read timeout
                ...
                catch (WebException ex)
                {
                    ...
                    {
                        LFSStats.WriteLine(" FAILED");
                        LFSStats.WriteLine("Could not load LFSWorld WRs (timeout or connection error).");
                        LFSStats.WriteLine("Continuing without WR data...");
                        LFSStats.ErrorWriteException("LFSWorld request failed after " + maxRequests + " attempts.", ex);
                        break;
                    }

                    LFSStats.Write(" retry " + requestNumber + "/" + maxRequests);
                    Thread.Sleep(lfswRequestPeriod);
                }
                catch (Exception ex)
                {
                    LFSStats.WriteLine(" FAILED");
                    LFSStats.WriteLine("LFSWorld error: " + ex.Message);
                    LFSStats.WriteLine("Continuing without WR data...");
            ...
            if (json == null)
            {
                LFSStats.WriteLine("LFSWorld WRs not available - continuing without them.");
                return initialized;
            }
