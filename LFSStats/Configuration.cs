...
namespace LFSStatistics
{
	class Configuration
	{
		...
		private const string _generateGraphs = "generateGraphs";
		private const GraphOptions defGenerateGraphs = GraphOptions.dll;
		private GraphOptions generateGraphs;
		public GraphOptions GenerateGraphs { get { return generateGraphs; } private set { generateGraphs = value; } }
		...
		public Configuration(string configFilePath)
		{
			...
			// language
			if (!config.TryGetValue(_language, out language))
			{
			...
			// generate graphs
			if (!config.TryGetValue(_generateGraphs, out tmp) || !Enum.TryParse(tmp, ignoreCase, out generateGraphs))
			{
				ReportError(_generateGraphs, defGenerateGraphs, tmp);
				generateGraphs = defGenerateGraphs;
			}
			// PubStat ID key
