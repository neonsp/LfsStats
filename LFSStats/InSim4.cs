...
namespace InSim
{
	...
	public class Connect
	{
		...
		Queue<byte> packetBuffer = new Queue<byte>(2048);
		//private bool incompletePacket = false;	// unused
		private int connectingTimeOut = 290;	// quit after this number of tries
		private int connectingDelay = 750;		// delay between connections in milliseconds
		...
		/// <param name="isLocal">if set to <c>true</c> [is local]. Can be used with Insim ISI flags to make local buttons.</param>
		/// <param name="TCPmode">if set to <c>true</c> [TCP mode].</param>
		public void insimConnect(IPEndPoint ipEndPoint, string adminPassword, string nameApp, bool isLocal, bool TCPmode)
		{
			this.TCPmode = TCPmode;
			if (this.TCPmode)
				insimConnectTCP(ipEndPoint, adminPassword, nameApp, isLocal);
			else
				insimConnectUDP(ipEndPoint, adminPassword, nameApp, isLocal);
		}
		public void insimConnectTCP(IPEndPoint ipEndPoint, string adminPassword, string nameApp, bool isLocal)
		{
			int retry = 0;
			LFSStats.Write("TCP: " + ipEndPoint + " connecting...");
			tc = new TcpConnection.Connection(ipEndPoint);

			while (!tc.Connect())
			{
				if (++retry < connectingTimeOut)
				{
					Thread.Sleep(connectingDelay);
					LFSStats.Write(".");
				}
				else
				{
					//Console.WriteLine("\nPress any key to continue connecting.");
					//Console.ReadKey();
					//Console.Write("\bTCP: Connecting...");
					//retry = 0
					//goto retryConnect;
					LFSStats.WriteLine(" timeout\n");
					LFSStats.Exit(2);
				}
			}
			LFSStats.WriteLine(" OK\n");
			...
			{
				LFSStats.ErrorWriteException("insimConnectTCP: Connect.Send failed, forced shutdown.", ex);
				LFSStats.Exit(1);
			}
			byte[] recvPacket;
		...
		public void insimConnectUDP(IPEndPoint ipEndPoint, string adminPassword, string nameApp, bool isLocal)
		{
			int retry = 0;
			LFSStats.Write("UDP: " + ipEndPoint + " connecting...");
		retryConnect:
			try
			{
				uc = new UdpClient(ipEndPoint);
			}
			catch (Exception ex)
			{
				LFSStats.ErrorWriteException("insimConnectUDP: UdpClient failed, forced shutdown.", ex);
				LFSStats.Exit(1);
			}

			int localport = ((IPEndPoint)uc.Client.LocalEndPoint).Port;
			...
			{
				LFSStats.ErrorWriteException("insimConnectUDP: Connect.Send failed, forced shutdown.", ex);
				LFSStats.Exit(1);
			}
			...
			catch
			{
				if (++retry < connectingTimeOut)
				{
					System.Threading.Thread.Sleep(connectingDelay);
					LFSStats.Write(".");
					goto retryConnect;
				}
				else
		...
		{
#if false
			// Old LFSStats TCP mode that causes a crash with newer LFS, I think this #else is the copied code from Luron's v1.12.
			// It tries to solve the issues that are supposed to be overfilling of LFSs TCP buffer or something.
			// I think abandoning this custom INSIM handling would be better and use an InsimDotNet library instead.
			if (this.TCPmode)
				return ReceiveTCP();
			else
				return ReceiveUDP();
			...
			byte[] tmpPack;
			if (this.TCPmode)
				recvPacket = ReceiveTCP();
			else
				recvPacket = ReceiveUDP();
			if (recvPacket != null)
			{
				for (int idx = 0; idx < recvPacket.Length; idx++)
					packetBuffer.Enqueue(recvPacket[idx]);
			}
			if (packetBuffer.Count > 0)
			{
				byte lenRequired = packetBuffer.Peek();
				if (lenRequired <= packetBuffer.Count)
				{
					tmpPack = new byte[lenRequired];
					for (int idx = 0; idx < lenRequired; idx++)
						tmpPack[idx] = packetBuffer.Dequeue();
					return tmpPack;
				}
				else
					return new byte[0];	// HACK null, received packet too big?
			}
			else
				return new byte[0];	// HACK null, buffer empty
#endif
		}
...
#if DEBUG
			Console.WriteLine("Packet head: " + packetHead);
#endif
			return packetHead;
		}
	...
	public class Encoder
	{
		...
		static public byte[] MTC(int UCID, int PLID, string msg)
		{
			...
			byte[] packet = new byte[72];	// dafuq! this is wrong
			packet[0] = 72;
		...
		/// <summary>
		/// InSim Disconnect Request Encoder 
		/// </summary>
		/// <returns></returns>
		// OK in InSim 4
		static public byte[] TINY_Close()
		{
			return IS_TINY((byte)TypePack.ISP_TINY, 0, (byte)TypeTiny.TINY_CLOSE);
		}
...
// on the host (in IS_MSO) and not displayed on anyone's screen.
	...
	public class Decoder
	{
		...
		public class MCI
		{
			...
			public MCI(byte[] packet)
			{
				...
				for (int i = 0; i < System.Math.Min(8, numOfPlayers); i++)
				{
					...
					//                    Console.WriteLine("ID:{0} x:{1} y:{2} z:{3} speed:{4} direction:{5} heading:{6} angvel:{7}", PLID, x, y, z, speed, direction, heading, angvel);

					//                    if(debug)Console.WriteLine("{0:F2}m {1:F2}m {2:F2}m {3:F2}km/h {4:F2} {5:F2} {6:F2}", xm, ym, zm, spm, dirm, headm, angm);
				}
			}
		}
		// Ok for Insim 4
		...
		public class SPX
		{
			...
			{
				PLID = pakGetByte(pak, 3);
				splitTime = pakGetUnsigned(pak, 4);
				ETime = pakGetUnsigned(pak, 8);
		...
		public class RST
		{
			...
			public readonly int QualMins;
			public readonly int NumP;
			public readonly string TrackNameShort;
			public readonly int Weather;
			public readonly int Wind;
			public readonly int Flags;
			...
			public RST(byte[] pak)
			{
				...
				TrackNameShort = pakGetString(pak, 8, 6);
		...
		public class SMALL
		{
			public readonly int ReqI;
			public readonly int SubT;
			public readonly long UVal;

			public SMALL(byte[] pak)
			{
				ReqI = pakGetByte(pak, 2);
				SubT = pakGetByte(pak, 3);
				UVal = pakGetUnsigned(pak, 4);
			}
		}
